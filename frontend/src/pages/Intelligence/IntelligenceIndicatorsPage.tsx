      ...
      <aside className="cti-indicators-sidebar">
        <div className="cti-indicators-sidebar-inner">
          <div className="cti-indicators-logo">
            ...
            <h1 style={{ fontSize: "var(--text-16)", fontWeight: 700 }}>Consola SIEM/XDR</h1>
          </div>
          <div className="cti-indicators-nav">
            {["Dashboard", "Incidents", "Threat Intelligence", "Hunting", "Settings"].map((item) => (
              ...
              >
                ...
                <span style={{ fontSize: "var(--text-14)", fontWeight: 500 }}>{item}</span>
              </div>
            ))}
          </div>
          <div className="cti-indicators-user">
            <div style={{ width: 32, height: 32, borderRadius: 999, background: "linear-gradient(90deg,var(--palette-374151),var(--palette-4b5563))" }} />
            <div>
              <div style={{ fontSize: "var(--text-14)", fontWeight: 600, color: "var(--palette-fff)" }}>Analyst Jane</div>
              <div style={{ fontSize: "var(--text-12)", color: "var(--palette-6b7280)" }}>SOC Level 2</div>
            </div>
          </div>
        </div>
      </aside>
      ...
      <div className="cti-indicators-main">
        <header className="cti-indicators-topbar">
          <div className="cti-indicators-breadcrumbs">
            ...
            <span>/</span>
            <span style={{ color: "var(--palette-fff)", fontWeight: 500 }}>Indicators &amp; Observables</span>
          </div>
            ...
            <button type="button" aria-label="Notifications" style={{ position: "relative" }}>
              ...
              <span
                style={{
                  ...
                  height: 8,
                  borderRadius: 999,
                  background: "var(--palette-ef4444)",
                  border: "1px solid var(--palette-111418)",
                }}
              />
            </button>
        ...
        <main className="cti-indicators-content">
          <div className="cti-indicators-center">
            ...
            <div className="cti-indicators-search-row">
              <div className="cti-indicators-toggle">
                <button className="active" type="button">
                  Observables <span style={{ background: "var(--alpha-255-255-255-0_2)", padding: "2px 6px", borderRadius: 6, fontSize: "var(--text-12)" }}>{data.observablesCount}</span>
                </button>
                <button type="button">
                  Indicators <span style={{ background: "var(--palette-283039)", padding: "2px 6px", borderRadius: 6, fontSize: "var(--text-12)", color: "var(--palette-d1d5db)" }}>{data.indicatorsCount}</span>
                </button>
              </div>
            ...
            <div className="cti-indicators-action-bar">
              <div style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--palette-60a5fa)", fontWeight: 500 }}>
                <span className="material-symbols-outlined" style={{ fontSize: "var(--text-18)" }}>
                  check_box
                </span>
                1 Selected
              </div>
              <div style={{ width: 1, height: 16, background: "var(--palette-283039)" }} />
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button type="button" style={{ color: "var(--palette-f87171)" }}>
                  <span className="material-symbols-outlined" style={{ fontSize: "var(--text-18)" }}>
                    delete
                  </span>
                  Delete
                      ...
                      <tr key={row.id} className={`cti-indicators-row ${row.selected ? "selected" : ""}`}>
                        ...
                        <td>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            {row.selected ? (
                              <span className="material-symbols-outlined" style={{ fontSize: "var(--text-16)", color: "var(--cti-primary)" }}>
                                content_copy
                              </span>
                            ) : null}
                          </div>
                        ...
                        <td style={{ color: "var(--palette-d1d5db)" }}>{row.lastSeen}</td>
                        <td>
                          ...
                          {row.riskScore ? (
                            <div className="cti-indicators-risk">
                              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "var(--text-12)" }}>
                                <span style={{ color: row.riskSeverity === "low" ? "var(--palette-34d399)" : "var(--palette-f87171)", fontWeight: 700 }}>
                                  {row.riskLabel}
                                </span>
                                <span style={{ color: "var(--palette-9ca3af)" }}>{row.riskScore}/100</span>
                              </div>
                              <div className="bar">
                                <span
                                  style={{
                            ...
                            <span style={{ fontSize: "var(--text-12)", color: "var(--palette-9ca3af)" }}>
                              {row.enrichmentStatus === "enriching" ? "Pending..." : "N/A"}
                            </span>
                          )}
                        </td>
                        <td style={{ textAlign: "right" }}>
                          <button type="button" style={{ color: "var(--palette-6b7280)" }}>
          ...
          <aside className="cti-indicators-drawer">
            <div className="cti-indicators-drawer-header">
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: "var(--text-18)", fontWeight: 700 }}>{data.drawer.id}</span>
              </div>
              <button type="button" style={{ color: "var(--palette-9ca3af)" }}>
            ...
            <div className="cti-indicators-drawer-body">
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 16 }}>
                <div className="cti-indicators-card">
                  <div style={{ fontSize: "var(--text-12)", color: "var(--palette-9ca3af)", marginBottom: 4 }}>Risk Score</div>
                  <div style={{ display: "flex", alignItems: "flex-end", gap: 8 }}>
                    <span style={{ fontSize: "var(--text-24)", fontWeight: 700, color: "var(--palette-f87171)" }}>{data.drawer.riskScore}</span>
                    <span style={{ fontSize: "var(--text-12)", color: "var(--palette-f87171)" }}>{data.drawer.riskLabel}</span>
                  </div>
                </div>
                <div className="cti-indicators-card">
                  <div style={{ fontSize: "var(--text-12)", color: "var(--palette-9ca3af)", marginBottom: 4 }}>Confidence</div>
                  <div style={{ display: "flex", alignItems: "flex-end", gap: 8 }}>
                    <span style={{ fontSize: "var(--text-24)", fontWeight: 700 }}>{data.drawer.confidence}</span>
                    <span style={{ fontSize: "var(--text-12)", color: "var(--palette-9ca3af)" }}>High</span>
                  </div>
                </div>
              </div>

              <div>
                <h3 style={{ fontSize: "var(--text-12)", color: "var(--palette-6b7280)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>
                  Metadata
                </h3>
                <div className="cti-indicators-metadata">
                  {[
                    ...
                    <div key={item.label} className="cti-indicators-metadata-row">
                      <span style={{ color: "var(--palette-9ca3af)" }}>{item.label}</span>
                      <span style={{ color: "var(--palette-fff)" }}>{item.value}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="cti-indicators-tags">
                  ...
                  <button className="cti-indicators-tag" type="button" style={{ borderStyle: "dashed", color: "var(--palette-6b7280)" }}>
                    + Add
                  </button>
                </div>
              </div>
                ...
                {data.drawer.incidents.map((incident) => (
                  <div key={incident.id} className="cti-indicators-incident">
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      <span className="material-symbols-outlined" style={{ fontSize: "var(--text-16)", color: "var(--palette-ef4444)" }}>
                        warning
                      </span>
                      <span style={{ fontWeight: 600, color: "var(--palette-fff)" }}>{incident.id}</span>
                    </div>
                    <p style={{ fontSize: "var(--text-12)", color: "var(--palette-9ca3af)" }}>{incident.summary}</p>
                  </div>
                ))}
              </div>
            </div>
